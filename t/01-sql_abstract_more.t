...
apply_tests(0); # 0: no quoting
apply_tests(1); # 1: with quoting

sub apply_tests {
  my ($with_quoting) = @_;

  # sub for testing against either the unquoted SQL or the quoted SQL
  my $is_same_sql_bind = sub {
    my ($got_sql, $got_bind, $want_sql_unquoted, $want_sql_quoted, $want_bind, $tst_msg) = @_;
    $tst_msg //= "";

    $with_quoting ? defined $want_sql_quoted &&
                    SQL::Abstract::Test::is_same_sql_bind($got_sql, $got_bind, 
                                                          $want_sql_quoted,   $want_bind,
                                                          "$tst_msg - with quoting")
                  : defined $want_sql_unquoted &&
                    SQL::Abstract::Test::is_same_sql_bind($got_sql, $got_bind,
                                                          $want_sql_unquoted, $want_bind,
                                                          $tst_msg);
  };    

  # args when creating a SQLA instance
  my %quoting_args = $with_quoting ? (quote_char => q{"}, name_sep => q{.}) : ();
  
  # local vars for common use
  my ($sql, @bind, $join);
  ...
  # old API transmitted to parent - column as string
  ($sql, @bind) = $sqla->select('Foo', 'bar', {bar => {">" => 123}}, ['bar']);
  $is_same_sql_bind->(
    $sql, \@bind,
    "SELECT bar FROM Foo WHERE bar > ? ORDER BY bar",
    'SELECT bar FROM "Foo" WHERE ( "bar" > ? ) ORDER BY "bar"', # BUGGY SQL but the parent works that way
    [123],
    "old API (positional parameters), column as string",
  );

  # old API transmitted to parent - column as arrayref
  ($sql, @bind) = $sqla->select('Foo', ['bar'], {bar => {">" => 123}}, ['bar']);
  $is_same_sql_bind->(
    $sql, \@bind,
    "SELECT bar FROM Foo WHERE bar > ? ORDER BY bar",
  ...
  $is_same_sql_bind->(
    $sql, \@bind,
    "SELECT col1, col2,
           (SELECT COUNT(*) AS count_foo FROM Foo WHERE bar_id=Bar.bar_id and height BETWEEN ? AND ?) AS col3,
           col4
  ...
  ($sql, @bind) = $sqla->select(
         -from => [ -join => \$subq_a, #  'SELECT "id1" AS "id" FROM "table1" WHERE ("id1" > 2)|a',
                             {operator => '<=>',
                              using => [ 'id' ]},
                             \$subq_b, #  'SELECT "id2" AS "id" FROM "table2" WHERE ("id2" > 2 )|b',
                  ],
  ...
  $is_same_sql_bind->(
    $sql, \@bind,
    "SELECT foo, SUM(bar) AS sum_bar FROM Foo GROUP BY foo HAVING sum_bar > ?", 
    'SELECT "foo", SUM(bar) AS "sum_bar" FROM "Foo" GROUP BY "foo" HAVING "sum_bar" > ?',
    [10],
    "group by / having",
  );
  ...
  $is_same_sql_bind->(
    $sql, \@bind,
    "SELECT SUM(bar) AS sum_bar FROM Foo WHERE ( foo = ? ) HAVING ( sum_bar > ? )",
    'SELECT SUM(bar) AS "sum_bar" FROM "Foo" WHERE ( "foo" = ? ) HAVING ( "sum_bar" > ? )',
    [1,10],
    "group by / having (2)",
  );
  ...
  $is_same_sql_bind->(
    ...
    undef, # no test if $with_quoting
    [],
    "aliased cols with '|', single char on left-hand side, no parens"
  );
  ...
  $is_same_sql_bind->(
    ...
    'SELECT (NULL) AS "a1", (2) AS "a2", "x" AS "a3" FROM "Foo"',
    [],
    "aliased cols with '|', single char on left-hand side, parens"
  );
  ...
  $sqla = SQL::Abstract::More->new(sql_dialect => 'Oracle', %quoting_args);
  ($sql, @bind) = $sqla->select(
  ...
  $is_same_sql_bind->(
    ...
    'SELECT * FROM (SELECT subq_A.*, ROWNUM rownum__index FROM (SELECT * FROM "Foo") subq_A WHERE ROWNUM <= ?) subq_B WHERE rownum__index >= ?',
    [15, 6],
    "old Oracle hack for limit/offset",
  );


  # Oracle12c version of limit/offset
  $sqla = SQL::Abstract::More->new(sql_dialect => 'Oracle12c', %quoting_args);
  ($sql, @bind) = $sqla->limit_offset(123, 456);
  $is_same_sql_bind->(
    $sql, \@bind,
  ...
  $sqla = SQL::Abstract::More->new(%quoting_args, max_members_IN => 10);
  ...
  $is_same_sql_bind->(
    ...
    ' WHERE ( ( "foo" NOT IN ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ) '
        ...
        . ' AND "foo" NOT IN ( ?, ?, ?, ?, ?) ) )',
    [1 .. 35],
    "max members_IN, smaller",
  );

  $sqla = SQL::Abstract::More->new(%quoting_args, max_members_IN => 3);
  ...
  my $sqla_RO = SQL::Abstract::More->new(%quoting_args, select_implicitly_for => 'READ ONLY');
